...
2. Create an organization in Devant.
    ...
    ![Create an organization in Devant](../assets/img/administer/create-devant-organization.png)
    ...
    3. Select the entity type to apply the domain name.
        ...
        !!! tip
             In this context:
              ...
              - The **API** entity type represents Devant components exposed via an endpoint, including API Proxy, Service, and Webhook components.
    ...
    !!! note "If you want to import your own certificate, it should adhere to specific guidelines"
          - TLS certificate guidelines:
             ...
             - It should be issued directly or through a wildcard entry for the provided custom URL. For example,
                - For direct issuance, the SSL file must include the exact domain name. For example, if the domain is `apis.devant.dev`, the SSL file must include `apis.devant.com`.
                - For wildcard entries, the SSL file should use a wildcard notation to cover all subdomains under the provided URL. For example, if the CNAME is `apis.devant.dev`, the SSL file should use `*.devant.dev`.
