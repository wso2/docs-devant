...
!!! info "Note"
    ...
    In the free tier, integrations run in a single-replica, low-availability mode.

![Scale integration replicas](../../assets/img/devops-and-ci-cd/scaling/scaling-view.png){.cInlineImage-full}

The following parameters allow you to scale Integration replicas:
...
!!! tip
    If you update a scaling parameter, it may not immediately reflect in the Devant Console because the change can take some time to propagate.

!!! info "Run a fixed number of replicas"
    If you want to run exactly `3` replicas for a integration, you must set the minimum and maximum replicas to `3`.

!!! warning "Scale to zero"
    - Although it is possible to set the minimum number of replicas to `0`, your integration does not scale to zero automatically during low usage. It can only go down to `1` replica.
